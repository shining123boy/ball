...
class Main extends egret.DisplayObjectContainer {
    ...
    */
     ...
     * const
     */
    private stageWidth: number;// = this.stage.stageWidth;
    private stageHeight: number;// = this.stage.stageHeight;
    private circleX: number;
    private circleY: number;
    private radius: number = 120;
    private lines = new Array();
    private balls = new Array();
    private shotBalls = new Array();
    private is_twBall_arr: Array<boolean> = new Array();
    ...
    private speedTimer: egret.Timer;
    ...
    private createGameScene(): void {
        ...
        var sky: egret.Bitmap = this.createBitmapByName("bg06");
        sky.width = 640;
        sky.height = 800;
        this.addChild(sky);
        ...
        // 菜单点击事件
        var menu: egret.Bitmap = this.createBitmapByName("menu_png");
        this.addChild(menu);
        // 菜单点击事件
    ...
    private rotateAll(speed: number) {
    ...
    private shotBall() {
        ...
        if (this.balls.length > 0) {
            ...
            for (var i = 0; i < this.balls.length; i++) {
                this.balls[i].y -= 30;
                // var ball_up = egret.Tween.get(ball_item);//balls.pop());//ball_item);
                // tw_ball.to({ y: 470 }, 300);// 插针的速度，控制难易
            }
            //注册事件          
            ...
            tw_ball.call(function () {
                ...
                // 生成线和球插在大球上
                var line_item: Line = new Line(0, 120, ball_item.getColor(), ball_item.getNumber());//this.rotateLine(this.lines[i-1],i*2*Math.PI/3);
                ...
                if (this.balls.length == 0 && this.is_twBall_arr.length == 0) {//保证异步执行的顺序
                    // 记录结束时间
    ...
    private nextLevel() {
        ...
        egret.Tween.removeAllTweens();//删除所有动画
        var succMenu: egret.Bitmap = this.createBitmapByName("res_suc_png");
        ...
        // next level button
        var nextLevel: egret.Bitmap = this.createBitmapByName("nextlevel");
    ...
    private loseMenu() {
        ...
        var menuBg: MenuBg = new MenuBg();
        this.addChild(menuBg);
        var loseMenu: egret.Bitmap = this.createBitmapByName("res_fail_png");
        ...
        // 重新挑战
        var redo: egret.Bitmap = this.createBitmapByName("redo_png");
    ...
    private initBalls() {
        ...
        var stageH: number = this.stage.stageHeight;
        var circlex: number = stageW / 2;
        var circley: number = stageH / 2 - 50;
        //this.baseball = 8;
        //console.log("baseball:" + this.baseball);
        for (var i = 1; i <= this.baseball; i++) {
            // this.removeChild();
            var ball_item: ball = new ball(0, 0, 12, i - 1);//circlex,stageH-20,20);
            ball_item.x = circlex;
            if (this.baseball < 6) {
    ...
    public drawBaseLine() {
        ...
        for (var i = 0; i < this.basicLine; i++) {
            var line_item: Line = new Line(0, 120, 0x000000, "0");//this.rotateLine(this.lines[i-1],i*2*Math.PI/3);
            line_item.x = this.circleX;
            line_item.y = this.circleY;
    ...
    public addScore(currScore: number) {
        // score
    ...
    public createMenuItem() {
        var menuBg: MenuBg = new MenuBg();
        this.addChild(menuBg);
        var menu: egret.Bitmap = this.createBitmapByName("menulayer_bg_png");
    ...
    public getScore(level: number, redoCount: number, timediff: number): number {
